...
class EnhancedAzureSQLDatabase:
    ...
    def __init__(self, config: DatabaseConfig):
        ...
        self.schema = config.schema_config.name
    ...
    def setup_schema(self) -> bool:
        ...
        with self.get_connection() as conn:
            ...
            try:
                if self.config.schema_config.table_behavior == 'drop_and_create':
                    self._drop_all_tables(cursor)
                    self._create_all_tables(cursor)
                elif self.config.schema_config.table_behavior == 'create_if_not_exists':
                    self._create_tables_if_not_exist(cursor)
                elif self.config.schema_config.table_behavior == 'append_only':
                ...
                if self.config.schema_config.table_behavior in ['drop_and_create', 'create_if_not_exists']:
    ...
    def _verify_tables_exist(self, cursor) -> bool:
        ...
        for table_name, enabled in self.config.schema_config.tables.items():
            if enabled:
    ...
    def _create_tables_if_not_exist(self, cursor):
        ...
        for table_name, table_sql in tables.items():
            if self.config.schema_config.tables.get(table_name.replace(self.config.schema_config.table_prefix, ''), True):
    ...
    def _drop_all_tables(self, cursor):
        ...
        for table_name in tables_to_drop:
            if self.config.schema_config.tables.get(table_name, True):
                full_table_name = self.config.get_table_name(table_name)
    ...
    def _create_all_tables(self, cursor):
        ...
        for table_name, table_sql in tables.items():
            base_name = table_name.replace(self.config.schema_config.table_prefix, '')
            if self.config.schema_config.tables.get(base_name, True):
                self.logger.info(f"Creating table: {table_name}")
                cursor.execute(table_sql)
    ...
    def _get_table_definitions(self) -> Dict[str, str]:
        ...
        if self.config.schema_config.tables.get('financial_profiles', True):
